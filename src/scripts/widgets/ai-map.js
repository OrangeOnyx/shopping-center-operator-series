export function renderAiMap(el, data) {
  ...
  [['helps', data.helps, 'cc-tag-success'], ['humans', data.humans, 'cc-tag-ink']].forEach(([key, col, tagClass]) => {
    const colEl = document.createElement('div');
    colEl.className = 'ai-map-col ai-map-' + key;
    colEl.innerHTML = `<div class="ai-map-col-head"><span class="cc-tag ${tagClass}">${col.label}</span></div>`;
    col.cards.forEach((c) => {
